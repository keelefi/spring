...
bool CStrafeAirMoveType::Update()
{
	...
	switch (aircraftState) {
		...
		case AIRCRAFT_CRASHING: {
            ...
            {
                LOG_L(L_NOTICE, "ForcedKillUnit() from StrafeAirMoveType, aircraft crashing - id: %d, %s", owner->id, __func__);
...
bool CStrafeAirMoveType::HandleCollisions(bool checkCollisions) {
	...
	if (pos != oldPos) {
		...
		if (hitBuilding && owner->IsCrashing()) {
			...
			// the ground
            LOG_L(L_NOTICE, "ForcedKillUnit() from StrafeAirMoveType, aircraft hit building while crashing - id: %d, %s", owner->id, __func__);
			owner->ForcedKillUnit(nullptr, true, false, -CSolidObject::DAMAGE_AIRCRAFT_CRASHED);
			return true;
		}
